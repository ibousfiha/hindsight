...
export function loadConfig(pluginOptions?: Record<string, unknown>): HindsightConfig {
  ...
  const dynamicBankGranularityEnv = process.env["HINDSIGHT_DYNAMIC_BANK_GRANULARITY"];
  if (dynamicBankGranularityEnv !== undefined) {
    config["dynamicBankGranularity"] = dynamicBankGranularityEnv
      .split(",")
      .map((t) => t.trim())
      .filter(Boolean);
  }
